...
import com.github.dsaouda.fiap.hack.apostila.*
import com.github.dsaouda.fiap.hack.apostila.page.ArquivoPastaPage
import com.github.dsaouda.fiap.hack.apostila.page.DownloadPage
...
class ConteudoCursoParser(private val span: Element, private val diretorioDisciplina: File) {
    ...
    fun process() {
        ...
        if (spanApostila.size > 0) {
            ...
            val arquivos = ArquivoPastaPage().request(post)
            val file = File("${diretorioDisciplina.absolutePath}/${pasta}")
            if (!file.exists()) {
                file.mkdirs()
            }

            ArquivosParser(arquivos, file).process()
        }
        ...
        if (spanArquivo.size > 0) {
            ...
            when {
                paramDownlaod.trim().length == 0 -> {
                    ...
                    if (!file.exists()) {
                        val conteudo = DownloadPage().downloadLink(link)
                    ...
                    val data = extrairData(span.select(".i-apostilas-link-subtitle").text())
                    DownloadPage().download(diretorioDisciplina, paramDownlaod, arquivo, data)
                }
            }
        }
    }
